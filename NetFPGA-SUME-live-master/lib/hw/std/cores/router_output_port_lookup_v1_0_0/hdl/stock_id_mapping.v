...
*******************************************************************************/
  ...
  module stock_id_mapping
   ...
   (// --- Interface to the previous stage
    ...
    //output [198:0] order_out,
    output [234:0] order_out,      
...
//   reg  [198:0] order_in;
   ...
   reg [19:0]			   price_reg_1; //900000
   reg [16:0]			   price_reg_2; //90000
   reg [13:0]			   price_reg_3; //9000
   reg [9:0]			   price_reg_4; //900
   reg [6:0]			   price_reg_5; //90
   reg [3:0]			   price_reg_6; //9
...
//------------------------- Modules-------------------------------
   ...
   //-------------------------------------
    fallthrough_small_fifo #(.WIDTH(235), .MAX_DEPTH_BITS(8)) // 4
    //fallthrough_small_fifo #(.WIDTH(199), .MAX_DEPTH_BITS(4)) // 4
      parse_input_fifo
...
// [198:103] => OrigClOrdID, [102:63]  => OrderID,[62:15] => Symbol, [14:13] => Side, [12:0] => Twsefields

// [234:139] => OrigClOrdID, [138:99]  => OrderID,[98:51] => Symbol, [50:49] => Side, [48:13]=> Price [12:0] => Twsefields
    always @(posedge clk) begin
        if(reset) begin
		counter <= 8'b0;
		//order_in <= 199'h0;
		order_in <= 235'h0; //36+199 = 235
		in_fifo_wr_order <= 'b0;
	end
	else begin
		if(valid) begin
		...
		if(counter == 8'd4) begin	
			//order_in[198:103] <= tdata[95:0];
			order_in[234:139]   <= tdata[95:0];
		end
		if(counter == 8'd5) begin	
			//order_in[102:15] <= {tdata[223:184], tdata[71:24]};
			order_in[138:51]   <= {tdata[223:184], tdata[71:24]};
		end
		if(counter == 8'd6) begin //Side & Price
			//order_in[14:13]	<= tdata[145:144]; //side
/*
			order_in[50:49]    <= tdata[145:144]; //side
			order_in[48:13]     <= {tdata[107:104],tdata[99:96],tdata[91:88],tdata[83:80],tdata[75:72],tdata[67:64],tdata[59:56],tdata[51:48],tdata[43:40]}; //price
*/

			order_in[50:13]     <= {tdata[145:144],tdata[107:104],tdata[99:96],tdata[91:88],tdata[83:80],tdata[75:72],tdata[67:64],tdata[59:56],tdata[51:48],tdata[43:40]}; //price
		end
		if(counter == 8'd7) begin
			order_in[12:10] <= tdata[26:24];//3	
		end
		if(counter == 8'd8) begin
			order_in[9:0] <= {tdata[153:152], tdata[95:88]};	
		end
		if(tlast && valid) begin
