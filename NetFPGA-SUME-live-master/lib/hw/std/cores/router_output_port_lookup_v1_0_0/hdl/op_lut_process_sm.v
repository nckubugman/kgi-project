...
//   output logon_sended,
   ...
   reg			 recv_tcp_report_ack;
   reg			 recv_tcp_report_ack_next;
   //  Keep connect  Alive
   ...
   assign is_fix_logon = is_ip_pkt  && is_logon;
   assign is_fix_report = is_ip_pkt && is_report;
   assign is_ack = is_ip_pkt && is_tcp_ack;
   assign send_ack_signal = send_ack_sig;

   assign tcp_report_ack_recieved = recv_tcp_report_ack;
...
//               if(is_ip_feed||is_udp_pkt||is_tcp) begin
                       ...
                       else if(ip_checksum_is_good && is_ip_feed  && udp_checksum_is_good ) begin
                          	state_next                  = SEND_PKT;
         			pkt_forwarded		    = 1;
         			//dst_port_next		    = 'h01;
...
/*
         	       else if(is_tcp_ack)begin
         		     if(is_send_pkt)begin
         		     	recv_tcp_report_ack_next = 1;
         		     end
         		     rd_preprocess_info = 1;
         		     in_fifo_rd_en = 1;
         		     state_next = DROP_PKT;
...
//	else if(send_pkt_counter == 32'h5F5E1000) begin //160000000
	else if(send_pkt_counter == 32'h39387000)  begin // 960000000
...
//                out_tkeep_next  = 32'hffffffe0;
   ...
   always @(posedge clk) begin
    ...
    //	 send_pkt_counter  <= 0;
	 ...
	 recv_tcp_report_ack	    <= recv_tcp_report_ack_next ;
      end // else: !if(reset)
...
//		 if(fix_connect_start)begin
			 ...
			 //else if(fix_report_recieved||tcp_report_ack_recieved)begin
