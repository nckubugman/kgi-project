...
void list_commodity(void) {
  ...
  char *str = "1101  ";
  FILE *fp;
  if ((fp=fopen("order_commodity.txt","a"))==NULL)
  {
                printf("Open Failed.\n");
                return;
  }
...
void list_order(void) {
  ...
  unsigned long long order_num, order_num_sell;
  unsigned long long without_order_num;
  unsigned long long order_1_a ,order_1_b; 
  unsigned long long  order_2_a ,order_2_b,order_2_c;
  unsigned long long  order_3_a ,order_3_b,order_3_c;
  unsigned long long  order_4_a ,order_4_b,order_4_c;
  unsigned long long order_5_a ,order_5_b,order_5_c;

  unsigned long long order_1_tmp,order_2_tmp,order_3_tmp,order_4_tmp,order_5_tmp;
 
  unsigned long long order_1 ,order_2,order_3,order_4,order_5;
//  unsigned long long hex_order_1 , hex_order_2, hex_order_3, hex_order_4,hex_order_5;
/*  
   int hex_1_a,hex_1_b;
    int hex_2_a,hex_2_b,hex_2_c;
    int hex_3_a,hex_3_b,hex_3_c;
    int hex_4_a,hex_4_b;
    int hex_5_a,hex_5_b,hex_5_c;
  */
  /*
    int quo_1_a,quo_1_b;
    int quo_2_a,quo_2_b,quo_2_c;
    int quo_3_a,quo_3_b,quo_3_c;
    int quo_4_a,quo_4_b;
    int quo_5_a,quo_5_b,quo_5_c;
    int rem_1_a,rem_1_b;
    int rem_2_a,rem_2_b,rem_2_c;
    int rem_3_a,rem_3_b,rem_3_c;
    int rem_4_a,rem_4_b;
    int rem_5_a,rem_5_b,rem_5_c;
*/
  ...
  if ((fp=fopen("order_list.txt","a"))==NULL)			
  {
		printf("Open Failed.\n");
		return;
  } 
  if ((fp_2=fopen("order_store_num.txt","a"))==NULL)
  {
                printf("Open Failed.\n");
                return;
  }
  ...
  //printf("%d\n", index);  
//  table_address = (uint32_t)SUME_OUTPUT_PORT_LOOKUP_MEM_ORDER_ID_MAPPING_0_ADDRESS;
  table_address = (uint32_t)SUME_OUTPUT_PORT_LOOKUP_MEM_ORDER_ID_STORE_0_ADDRESS;
  table_address = table_address | index;
  cmd = (uint32_t)READ_CMD;
    err=writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, table_address);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS);
    ...
    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_HI, &c);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_HI);
    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_LOW, &d);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_LOW);

    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_C_HI, &e);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_C_HI);
    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_C_LOW, &f);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_C_LOW);
   


    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_D_HI, &g);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_D_HI);
/*
    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_D_LOW, &h);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_D_LOW);


    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_E_HI, &i);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_E_HI);

    err=readReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_E_LOW, &j);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_E_LOW);
...
//    printf("%u | %u | %u | %u | %u | %u | %u | %u | %u \n",a,b,c,d,e,f,g,h,i);
    ...
    order_1_a = (a&(0x1fffffff)) ;//29
    order_1_b = (b&(0xffe00000)) ;//11

    order_2_a = (b&(0x001fffff)) ;//21
    order_2_b = (c&(0xffffe000)) ;//19

    order_3_a = (c&(0x00001fff)) ;//13
    order_3_b = (d&(0xffffffc0)) ;//27

    order_4_a = (d&(0x0000003f)) ;//6
    order_4_b = (e&(0xffffffff)) ;//32
    order_4_c = (f&(0xc0000000)) ;//2

    order_5_a = (f&(0x3fffffff)) ;//30
    order_5_b = (g&(0xffc00000)) ;//10
 

    // Decimal to ASCII
/*
     order_1 =  ((order_1_a<<11) | (order_1_b>>21)) ;

     order_2 =  ((order_2_a<<19) |(order_2_b>>13)) ;
     order_3 =  ((order_3_a<<27)| (order_3_b>>5)) ;
     order_4 =  ((order_4_a<<34) |((order_4_b<<2)|(order_4_c>>29))) ;
     order_5 =  ((order_5_a<<10) |(order_5_b>>22)) ;
*/
//     fprintf(fp,"order1: %llx || order2: %llx || order3: %llx || order4: %llx || order5: %llx   \n",order_1,order_2,order_3,order_4,order_5);

//     fprintf(fp,"order1: %llx%llx || order2: %llx%llx || order3: %llx%llx || order4: %llx%llx%llx || order5: %llx%llx   \n",order_1_a,order_1_b,order_2_a,order_2_b,order_3_a,order_3_b,order_4_a,order_4_b,order_4_c,order_5_a,order_5_b);
//   fprintf(fp,"%llx%llx%llx%llx%llx%llx%llx   \n" ,a ,b,c,d,e,f,g);

/*
   fprintf(fp,"order1: %c%c%c%c%c || order2: %c%c%c%c%c || order3: %c%c%c%c%c || order4: %c%c%c%c%c || order5: %c%c%c%c%c   \n",
    order_1>>32,(order_1&(0x00ffffffff))>>24,(order_1&(0x0000ffffff))>>16,(order_1&(0x000000ffff))>>8,(order_1&(0x00000000ff)),
    order_2>>32,(order_2&(0x00ffffffff))>>24,(order_2&(0x0000ffffff))>>16,(order_2&(0x000000ffff))>>8,(order_2&(0x00000000ff)),
    order_3>>32,(order_3&(0x00ffffffff))>>24,(order_3&(0x0000ffffff))>>16,(order_3&(0x000000ffff))>>8,(order_3&(0x00000000ff)),
    order_4>>32,(order_4&(0x00ffffffff))>>24,(order_4&(0x0000ffffff))>>16,(order_4&(0x000000ffff))>>8,(order_4&(0x00000000ff)),
    order_5>>32,(order_5&(0x00ffffffff))>>24,(order_5&(0x0000ffffff))>>16,(order_5&(0x000000ffff))>>8,(order_5&(0x00000000ff))
    );
*/
  
//    order_num = a&(0x00000e00);
/*
    order_num = (a&(0x0000000f));
    fprintf(fp_2,"(%d)  %u \n",index ,order_num);
*/
/*
    if(order_num)
...
//    order_num = (a&(0xffe00000))>>21;
    order_num = a >> 29 ;
    fprintf(fp_2,"(%d)  %u \n",index ,order_num);

      count = order_num+ count;
    }
...
//  table_address = (1<<12) | (uint32_t)SUME_OUTPUT_PORT_LOOKUP_MEM_ORDER_ID_MAPPING_0_ADDRESS;
  ...
  table_address = (1<<12) | (uint32_t)SUME_OUTPUT_PORT_LOOKUP_MEM_ORDER_ID_STORE_0_ADDRESS;
  table_address = table_address | index;
  cmd = (uint32_t)READ_CMD;
    err=writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, table_address);
    if(err) printf("0x%08x: ERROR\n", SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS);
...
//    printf("%u | %u | %u | %u | %u | %u | %u | %u | %u \n",a,b,c,d,e,f,g,h,i);
/*
    order_1_a = (a&(0x001fffff)) ;
    order_1_b = (b&(0x00ffffe0)) ;
    order_2_a = (b&(0x0000001f)) ;
    order_2_b = c;
    order_2_c = (d&(0x00ffe000)) ;
    order_3_a = (d&(0x00001fff)) ;
    order_3_b = e;
    order_3_c = (f&(0x00e00000)) ;
    order_4_a = (f&(0x001fffff)) ;
    order_4_b = (g&(0x00ffffe0)) ;
    order_5_a = (g&(0x0000001f)) ;
    order_5_b = h;
    order_5_c = (i&(0x00ffe000)) ;

    // Decimal to ASCII
    
     order_1 =  (order_1_a<<19) | (order_1_b>>5) ;
     order_2 =  ((order_2_a<<35) | ((order_2_b<<11)|(order_2_c>>13))) ;
     order_3 =  ((order_3_a<<27)| ((order_3_b<<3)|(order_3_c>>21))) ;
     order_4 =  (order_4_a<<19) | (order_4_b>>5) ;
     order_5 =  ((order_5_a<<35) | ((order_5_b<<11)|(order_5_c>>13))) ;
*/
//    fprintf(fp,"order1: %llx || order2: %llx || order3: %llx || order4: %llx || order5: %llx   \n",order_1,order_2,order_3,order_4,order_5);
/*
   fprintf(fp,"order1: %c%c%c%c%c || order2: %c%c%c%c%c || order3: %c%c%c%c%c || order4: %c%c%c%c%c || order5: %c%c%c%c%c   \n",
...
//    order_num_sell = (a&(0x00e00000))>>21;
    order_num_sell = a >> 29;
    fprintf(fp_2,"(%d)  %u \n",index ,order_num_sell);

      count_sell = order_num_sell+ count_sell;
...
void set_seq(void){
        ...
        printf("please enter the sequence number(only 1~999999):");
        scanf("%d", &wr_value);
